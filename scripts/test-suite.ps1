...
foreach ($file in $k8sFiles) {
    try {
        $yaml = Get-Content $file.FullName | ConvertFrom-Yaml -ErrorAction SilentlyContinue
        Write-Host "   ✓ $($file.Name) is valid YAML" -ForegroundColor Green
    } catch {
        # YAML module might not be available, just check basic structure
